...
//    }
    ...
    public boolean touchUp(int screenX, int screenY, int pointer, int button) {
        ...
        if (!bMove && bClick) {
            if (button == Input.Buttons.LEFT && fMouseX >= fPosX[0]
                    && fMouseX <= (fPosX[0] + arsprSongBox[0].getWidth()) && fMouseY >= fPosY[0]
                    && fMouseY <= (fPosY[0] + arsprSongBox[0].getHeight())) {
                nButtonMenuPress = 1;
                System.out.println("1");
            }
            if (button == Input.Buttons.LEFT && fMouseX >= fPosX[1]
                    && fMouseX <= (fPosX[1] + arsprSongBox[1].getWidth()) && fMouseY >= fPosY[1]
                    && fMouseY <= (fPosY[1] + arsprSongBox[1].getHeight())) {
                    game.nScreen = 2;
                    game.updateState();
                    

            }
            if (button == Input.Buttons.LEFT && fMouseX >= fPosX[2]
                    && fMouseX <= (fPosX[2] + arsprSongBox[2].getWidth()) && fMouseY >= fPosY[2]
                    && fMouseY <= (fPosY[2] + arsprSongBox[2].getHeight())) {
                nButtonMenuPress = 3;
    ...
    public void show() {
        ...
        img[0] = new Texture("MenuSongButton1.png");
